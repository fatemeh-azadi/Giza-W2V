...
my($_ROOT_DIR, $_CORPUS_DIR, $_GIZA_E2F, $_GIZA_F2E, $_MODEL_DIR, $_TEMP_DIR, $_CORPUS, $_WORD2VEC, $_BOUNDARY_WORDS_PHRASE,
...
sub run_giza {
	...
	&run_single_giza($___GIZA_F2E,$___E,$___F,
		     $___VCB_E,$___VCB_F,
		     $___CORPUS_DIR."/$___F-$___E-int-train.snt", "$___WORD2VEC.$___E", "$___WORD2VEC.$___F")
	    unless $___DIRECTION == 2;
	&run_single_giza($___GIZA_E2F,$___F,$___E,
		     $___VCB_F,$___VCB_E,
		     $___CORPUS_DIR."/$___E-$___F-int-train.snt", "$___WORD2VEC.$___F", "$___WORD2VEC.$___E")
...
sub run_single_giza {
    my($dir,$e,$f,$vcb_e,$vcb_f,$train,$w2v_e,$w2v_f) = @_;
    ...
    if (defined $_WORD2VEC){ $GizaDefaultOptions{"w2vs"} = $w2v_e;
			     $GizaDefaultOptions{"w2vt"} = $w2v_f; }
...
sub extract_phrase {
    ...
    else
    {
      ...
      $cmd .= " --BoundaryWords" if defined($_BOUNDARY_WORDS_PHRASE);
