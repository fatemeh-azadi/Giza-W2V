...
GLOBAL_PARAMETER(int,N_w2v,"nw2v","number of similar words considered for each source word in w2v",PARLEV_OPTHEUR,1000);
...
string Prefix, LogFilename, OPath, Usage, 
  ...
  n_Filename, dictionary_Filename, sourceWord2Vec_Filename, targetWord2Vec_Filename;
...
{ 
  ...
  Word2Vec *word2vec = new Word2Vec(N_w2v);
  if(useWord2Vec) word2vec->readWord2Vec(sourceWord2Vec_Filename, targetWord2Vec_Filename, &eTrainVcbList, &fTrainVcbList);
  ...
  model1 m1(CorpusFilename.c_str(), eTrainVcbList, fTrainVcbList,tTable,trainPerp, 
   ...
   if(ReadTablePrefix.length() )
     ...
     minIter=m1.em_with_tricks(Model1_Iterations,seedModel1,*dictionary, useDict, *word2vec, useWord2Vec);
	 errors=m1.errorsAL();
       }
...
int main(int argc, char* argv[])
{
  ...
  getGlobalParSet().insert(new Parameter<string>("W2V_S",ParameterChangedFlag,"source word2vec file name",sourceWord2Vec_Filename,PARLEV_INPUT));
  getGlobalParSet().insert(new Parameter<string>("W2V_T",ParameterChangedFlag,"target word2vec file name",targetWord2Vec_Filename,PARLEV_INPUT));
